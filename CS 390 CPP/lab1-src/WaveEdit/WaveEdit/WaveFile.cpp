...
WaveFile * WaveFile::get_fragment(double startms, double endms) {
	WaveFile * nWave = new WaveFile(numChannels, sampleRate, bitsPerSample);

	double mStartms = startms * (this->sampleRate / 1000);
	double mEndms = endms * (this->sampleRate / 1000);

	if (mStartms > mEndms) {
		double temp = mStartms;
		mStartms = mEndms;
		mEndms = temp;
	}

	for (int i = mStartms; i < mEndms; i++) {
...
WaveFile * WaveFile::remove_fragment(double startms, double endms) {
	...
	for (int i = 0; i < mStartms; i++) {
		nWave->add_sample(this->get_sample(i));
	}

	for (int i = mEndms; i < this->lastSample; i++) {
		nWave->add_sample(this->get_sample(i));
	}
...
WaveFile * WaveFile::paste_fragment(double startms, WaveFile * clipboard) {
	WaveFile * nWave = new WaveFile(numChannels, sampleRate, bitsPerSample);
	...
	for (int i = 0; i < clipboard->lastSample; i++) {
		nWave->add_sample(clipboard->get_sample(i));
	}
	for (int i = mStartms; i < this->lastSample; i++) {
		nWave->add_sample(this->get_sample(i));
	}
...
WaveFile * WaveFile::replace_fragment(double startms, double endms, WaveFile * toModify) {
	WaveFile * nWave = new WaveFile(numChannels, sampleRate, bitsPerSample);
	...
	for (int i = 0; i < toModify->lastSample; i++) {
		nWave->add_sample(toModify->get_sample(i));
	}
	for (int i = mEndms; i < this->lastSample; i++) {
		nWave->add_sample(this->get_sample(i));
	}
...
WaveFile * WaveFile::changeSpeed(float speed) {
	WaveFile * nWave = new WaveFile(numChannels, sampleRate, bitsPerSample);
	double r = 0;
	int j = 0;
	int i;
	...
	while (r < this->lastSample) {
		i = (int)r;
		nWave->add_sample(get_sample((int)i));
		r = r + speed;
		j++;
	}
